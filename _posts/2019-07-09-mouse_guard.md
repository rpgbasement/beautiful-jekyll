...
> Листы персонажей делались с расчетом на стартер, проблему заметили, но добавлять в них что-то — нужно перекраивать раскладку и скорее всего что-то выкидывать. Чуть позже, когда делали листы для мастера, решили исправить этот момент в нём. Даже если есть карты, состояния могут переходить на следующую сессию и это лучше где-то отмечать.
